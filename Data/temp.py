import pandas as pd
import json
data = pd.read_csv('new5.csv', index_col=None)
...
freq_chord = data['Frequency_map_of_each_chord'].to_list()
f = open("Tabsultimatedata.csv", 'a+')
# print(freq_chord[0])
...
data.drop(['Frequency_map_of_each_chord'], axis=1, inplace=True)

brics = pd.DataFrame.to_csv(data, 'new6.csv')
